...
assert review['required_approving_review_count'] == 0, 'Sole owner cannot approve own PR'
assert not review['require_code_owner_review']
assert not review['require_last_push_approval']
assert all(review[key] for key in ('dismiss_stale_reviews_on_push', 'required_review_thread_resolution'))
...
assert re.search(r'^\*\s+@Shawdaimarie\s*$', owners, re.M), 'All paths belong to the sole human owner'
